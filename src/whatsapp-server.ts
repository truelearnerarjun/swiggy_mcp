...
<body>
  ...
  app.post("/webhook", async (req: Request, res: Response) => {
    ...
    if (lowerMsg === "/logout" || lowerMsg === "logout" || lowerMsg === "/disconnect" || lowerMsg === "disconnect") {
      ...
      await sendReply(sender, logoutMsg);
      return;
    }
    ...
    if (!accessToken) {
      if (!OAUTH_PUBLIC_BASE_URL) {
        await sendReply(sender, "⚠️ Swiggy connection service is not configured yet. Please contact the bot owner.");
        return;
      }
      try {
...
After connecting, return right here and tell me what you'd like to eat!`;
        await sendReply(sender, welcomeMsg);
      } catch (err: any) {
        console.error("Could not start Swiggy OAuth:", err?.message ?? err);
        await sendReply(sender, "⚠️ I could not generate your Swiggy connection link. Please try again shortly.");
      }
      return;
    }
